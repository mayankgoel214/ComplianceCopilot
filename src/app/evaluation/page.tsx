...
function MetricsTable({
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-muted-foreground">This slice was not measured.</p>;
  }
        ...
        <tbody className="tabular-nums">
          {rows.map((config) => {
            ...
            return (
              <tr
                key={config.name}
                className={`border-b border-border/40 ${isBest ? "bg-emerald-500/5" : ""}`}
              >
                <td className="py-2 pr-4 font-medium">
                  {config.name}
                  {isBest ? <span className="text-emerald-400 ml-2 text-xs">best</span> : null}
                </td>
                ...
                <td className="py-2 text-right text-muted-foreground">{m.medianLatencyMs} ms</td>
              </tr>
            );
          })}
        </tbody>
    ...
    <div className="mx-auto max-w-4xl px-5 py-10 space-y-12">
      <header className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <code className="text-xs bg-muted px-1.5 py-0.5 rounded">eval/run-eval.mts</code> against
          the committed index and the committed gold set. The prose version, with the full method
          and limitations, is in{" "}
          <code className="text-xs bg-muted px-1.5 py-0.5 rounded">docs/retrieval-eval.md</code>.
        </p>
      </header>
          ...
          {results.fusionWeightSweep.map((s) => (
            <div key={s.lexical} className="flex items-center gap-3 text-sm">
              ...
              <div className="flex-1 h-5 bg-muted/40 rounded-sm overflow-hidden">
                <div
                  className={`h-full ${
                    s.ndcg === maxSweep ? "bg-emerald-500/60" : "bg-primary/40"
                  }`}
